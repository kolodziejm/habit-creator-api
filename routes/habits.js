    ...
    try {
      ...
      if (userDaysDiff >= 1 && user.lastActiveDate !== null) {
        ...
        if (habitsArray.some(habit => habit.streak >= 7 && !habit.isFinished)) {
          failAchievValue += await handleAchievement(
            "I don't feel like doing it today",
            req.user.id
          );
        }
        await Habit.updateMany(
          { userId: req.user.id, isFinished: false },
          { streak: 0 }
        );
...
router.post(
  ...
  [
    check("name")
      .not()
      .isEmpty()
      .withMessage("Enter a habit name")
      .trim()
      .isLength({ max: 100 })
      .withMessage("Habit name has to be below 100 characters"),
    check("color")
      .not()
      .isEmpty()
      .withMessage("Choose a color")
      .isLength({ min: 4, max: 7 })
      .withMessage("Incorrect color format"),
    check("difficulty")
      .not()
      .isEmpty()
      .withMessage("Difficulty not selected")
      .isIn(["easy", "medium", "hard"])
      .withMessage("Wrong difficulty")
  ],
  async (req, res, next) => {
    ...
    try {
      ...
      if (currentHabits.length >= 10) {
        return res
          .status(400)
          .json({ name: "Maximum of 10 habits are allowed" });
      }
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const journeyAchCoins = await handleAchievement(
        "Journey has begun",
        req.user.id
      );
        ...
        case 3:
          {
            ...
            achievementCoins += await handleAchievement(
              "Freshman",
              req.user.id
            );
          }
      ...
      await user.save();
      res.json({
        msg: "Habit saved as finished!",
        coins: user.coins,
        value,
        bonus: bonus * (habit.streak - 1),
        achievementCoins
      });
    } catch (err) {
      console.log(err);
